...
`js/config.js` の `FEATURES.PASSWORD_RESET_EMAIL` が **`false`** のため、
ログイン画面の「パスワードをお忘れの方」は
**管理者への連絡案内のみ**を表示します。メールアドレスの入力欄は出ません。

これは意図的な設定です。外部SMTPを設定しないうちにメール送信を有効にすると、
利用者が入力しても「送信しました」と出るだけで**1通も届かず**、
届かないメールを待ち続けることになるためです。

必要な作業は **`migrations/20260811120052_021_admin_reset_password.sql` の適用のみ**です。

### メールによる再設定を有効にしたくなったら

1. 下記「1. Redirect URL の登録」と「2. SMTP の設定」を完了させる
2. `js/config.js` の `FEATURES.PASSWORD_RESET_EMAIL` を `true` に変更する

コードの変更はこの1箇所だけです。画面は自動的にメール入力欄に切り替わります。
...
| 経路 | 使う人 | 状態 | 必要な設定 |
|---|---|---|---|
| 管理者が直接リセット | 管理者 | **有効** | `migrations/20260811120052_021_admin_reset_password.sql` の適用 |
| メールで本人が再設定 | 全員 | 無効 | 下記1・2＋設定フラグを `true` に |

以下は、将来メール経路を有効にする場合の手順です。
**当面は読み飛ばして構いません。**
...
`migrations/20260811120052_021_admin_reset_password.sql` の管理者リセットで運用してください。
20名規模なら十分に回ります。
